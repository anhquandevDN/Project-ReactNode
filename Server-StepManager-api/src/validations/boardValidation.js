...
const createNew = async (req, res, next) => {
  /**
   * Note (vd 52): Mặc định chúng ta không cần phải custom message ở phía BE làm gì vì để cho Front-end tự validate và custom message phía FE cho đẹp.
  ...
  const correctCondition = Joi.object({
    title: Joi.string().required().min(3).max(50).trim().strict().messages({
      'any.required': 'Title is required (anhquandev)',
      'string.empty': 'Title is not allowed to be empty (anhquandev)',
      'string.min': 'Title length must be at least 3 characters long (anhquandev)',
      'string.max': 'Title length must be less than or equal to 5 characters long (anhquandev)',
      'string.trim': 'Title must not have leading or trailing whitespace (anhquandev)'
    }),
