...
const getBoards = async (userId, page, itemsPerPage, queryFilters) => {
  try {
    const queryConditions = [
      ...
      { _destroy: false },
      // điều kiện 2: cái thằng userId nó phải thuộc vào một trong 2 cái mảng ownerIds hoặc memberIds, sử dụng toán tử $all của mongodb
      { $or: [
        { ownerIds: { $all: [new ObjectId(userId)] } },
        { memberIds: { $all: [new ObjectId(userId)] } }
      ] }
    ...
    // Xử lý query filter cho trường hợp search board, ví dụ search theo title
    // Note trường hợp tiếng Việt trong MongoDB thì hiện tại nó chưa hỗ trợ, nhưng đặc thù dự án cần thì mọi người có thể dùng giải pháp thay title thành slug là được, phía FE vẫn đẩy lên q[title] nhưng phía BE mình bắt và chuyển hết tiếng Việt về không dấu rồi tìm theo slug
    if (queryFilters) {
      Object.keys(queryFilters).forEach(key => {
        // queryFilters[key] nó tương đương queryFilters[title] nếu phía FE đẩy lên q[title]
      ...
      [
        ...
        { $facet: {
          ...
          'queryBoards': [
            ...
            { $limit: itemsPerPage } // giới hạn tối đa số lượng bản ghi mỗi lần lấy trên một trang
          ],
          // Luồng thứ hai: Querry đếm tổng số lượng bản ghi board và trả về biến countedBoards
...
// Đẩy một phần tử userId vào cuối mảng memberIds
// Dùng $push trong mongodb ở trường hợp này để đẩy 1 phần tử vào cuối mảng
